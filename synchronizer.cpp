    ...
    synchronizer(){
        m_sync = {{
            -1, [](const QSet<QString>& aGroups, const QString& aIDE){
                for (auto i : aGroups)
                    rea2::pipeline::instance()->find("QSGAttrUpdated_reagrid" + i + aIDE)
                          ->removeNext("sync_QSGAttrUpdated_reagrid" + i + aIDE);
            }},
            {1, [this](const QSet<QString>& aGroups, const QString& aIDE){
                 for (auto i : aGroups)
                    rea2::pipeline::instance()->find("QSGAttrUpdated_reagrid" + i + aIDE)
                        ->nextF<QJsonArray>([i, this, aIDE](rea2::stream<QJsonArray>* aInput){
                            ...
                            for (auto i : mdys0){
                                ...
                                if (!mdy.contains("secondTrig")){
                                    mdy.remove("id");
                                    mdys.push_back(rea2::Json(mdy, "secondTrig", true));
                                }
                            }
                            if (mdys.size()){
                                auto cur = i.toInt();
                        ...
                        }, "", rea2::Json("name", "sync_QSGAttrUpdated_reagrid" + i + aIDE));
             }}
        };

        rea2::pipeline::instance()->find("reagridCountChanged")->nextF<double>([this](rea2::stream<double>* aInput){
        ...
        rea2::pipeline::instance()->add<QString>([this](rea2::stream<QString>* aInput){
            ...
            auto prm = rea2::Json("title", "synchronize");
            QJsonObject cnt;
            for (auto i = 0; i < m_sum; ++i){
                auto idx = QString::number(i);
                cnt[idx] = rea2::Json("value", last_sync.value(idx).isNull() ? 1 : last_sync.value(idx).toInt());
            }
            prm.insert("content", cnt);
            last_sync = rea2::in(prm, "", nullptr, true)->asyncCall("c++_setParam")->data();
            m_last_sync.insert(dt, last_sync);

            QHash<int, QSet<QString>> grps;
            for (auto i : last_sync.keys())
                rea2::tryFind(&grps, last_sync.value(i).toInt())->insert(i);
            for (auto i : grps.keys())
                if (m_sync.contains(i))
                    m_sync.value(i)(grps.value(i), dt);
                else
                    m_sync.value(- 1)(grps.value(i), dt);
        }, rea2::Json("name", "_Synchronize"));
    }
private:
